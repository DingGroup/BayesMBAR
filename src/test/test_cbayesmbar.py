...
def setup_data_cbmbar():
    ...
    for s1, s2 in pairs:
        ...
        u = np.stack([np.sum(0.5 * k[s] * (xs - mu[s]) ** 2, axis=1) for s in key], axis=0)
        num_conf = np.array([u.shape[1] // u.shape[0] for _ in range(u.shape[0])])

        u_list.append(u)

    num_conf_list = [[u.shape[1] // u.shape[0] for i in range(u.shape[0])] for u in u_list]
    identical_states = (
        [(0, 0), (3, 4), (4, 0)],
        [(0, 4), (1, 0), (5, 0)],
        [(1, 4), (2, 0), (4, 4)],
...
def test_CBayesMBAR(setup_data_cbmbar):
    u_list, num_conf_list, identical_states, pairs, deltaF_ref = setup_data_cbmbar
    cbmbar = CBayesMBAR(u_list, num_conf_list, identical_states, random_seed=0)
